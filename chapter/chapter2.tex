...
\begin{equation}
	...
	\label{chapter2:角动量1}
\end{equation}
其中$\delta \varOmega_{ik}$为该无穷小线性变换的系数，其所有分量构成一个四维张量。四维张量$\delta\varOmega_{ik}$的各个分量的取值需满足：矢径的长度在该转动下不变，即$x'^ix'_i = x^ix_i$，将式\eqref{chapter2:角动量1}的结果代入并略去二阶小量，可得
\begin{equation*}
...
对于封闭系统，Lagrange函数是一个不变量，它不因四维空间中的转动而改变，因此对任意的转动，即任意的四维反对称张量$\delta\varOmega_{ik}$都有$\delta S=0$，这说明式\eqref{chapter2:角动量4}中$\delta\varOmega_{ik}$的系数必须为零，即
\begin{equation}
